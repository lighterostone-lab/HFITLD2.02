...
Window_MenuStatus.prototype.drawActorExp = function(actor, x, y, width) {
	...
	// VENO EXP BAR FIX START
	var alv = actor.level;
	if (alv === actor.maxLevel()) return 1.0;
	var nre = actor.expForLevel(alv + 1) - actor.expForLevel(alv);//nre must be next required exp, veno is good at math i say!
	var ce = actor.currentExp() - actor.expForLevel(alv);//ce stands for current exp, tricky veno! :D
	// VENO EXP BAR FIX END
	var perc = (1.0 * ce / nre);//percentage% of current exp(0.8)
	var perc_100 = perc*100;//(80%)
	this.drawGauge(x, y, width, perc, color1, color2);
	this.changeTextColor(this.textColor(0));
	this.drawText(perc_100.toFixed(1)+"%", x, y, width, 'right');
	this.drawText(toNextLevel, x, y, width, 'left');
	this.resetTextColor();
};
...
//	var r = {'0': 0, '1': x, '2': x2, '3': x3, '4': x4, '5': x5};
	var r = {'0': 0, '1': 20, '2': 180, '3': 480, '4': x4, '5': x5};//每一列的左右位置
	var c = {'0': 0, '1': y, '2': y + lineHeight, '3': y + lineHeight * 2, '4': y + lineHeight * 3, '5': y + lineHeight * 4};//每一行的上下位置
	...
	if(showHP && Number(locationHP[1]) <= eval(statusRows) && Number(locationHP[2]) <= cols) this.drawActorHp(actor, r[locationHP[2]], c[locationHP[1]], 280);
	if(showMP && Number(locationMP[1]) <= eval(statusRows) && Number(locationMP[2]) <= cols) this.drawActorMp(actor, r[locationMP[2]], c[locationMP[1]], 280);
	if(showTP && Number(locationTP[1]) <= eval(statusRows) && Number(locationTP[2]) <= cols) this.drawActorTp(actor, r[locationTP[2]], c[locationTP[1]], 280);
	if(showEXP && Number(locationEXP[1]) <= eval(statusRows) && Number(locationEXP[2]) <= cols) this.drawActorExp(actor, r[locationEXP[2]], c[locationEXP[1]], 280);
	...
	for(var i = 0; i < customEvals.length; i++) {
		if(Number(customLocations[i][1]) <= eval(statusRows) && Number(customLocations[i][2]) <= cols) {
			...
			if(customCodes[i]) {//If the third parameter is TRUE
				this.drawTextEx(eval(code), r[customLocations[i][2]], c[customLocations[i][1]]);//text,x,y
			} else {
